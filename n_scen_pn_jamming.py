...
# Initialize communication channel and Pulsed Noise jammer
channel = ADSBChannel()
jammer = Jammer(jamming_probability=0.0, noise_intensity=0.1)  # Start with low jamming effect
...
def update(frame):
    active_drones = False
    jammer.jamming_probability = 0.5 * (np.sin(frame / 10) + 1)  # Pulsating jamming effect
    jammer.noise_intensity = 0.5 * (np.sin(frame / 10) + 1)  # Pulsating noise intensity
    ...
    for drone in drones:
        ...
        if status == -2:
            ...
            # Apply Pulsed Noise Jamming
            jammed_message, is_jammed = jammer.jam_signal(original_message)
            if is_jammed:
                print(f"Drone {drone.id} communication jammed! Message lost.")
