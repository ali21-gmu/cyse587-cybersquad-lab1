...
# Initialize communication channel and Sweeping Jammer
channel = ADSBChannel()
...
def update(frame):
    active_drones = False
    jammer.jamming_probability = 0.5 * (np.sin(frame / 5) + 1)  # Sweeping jamming effect
    jammer.noise_intensity = 0.5 * (np.cos(frame / 5) + 1)  # Sweeping noise intensity
